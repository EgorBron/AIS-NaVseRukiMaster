...
Процедура ОбработкаПроведения(Отказ, Режим)
	// Честно говоря, этот модуль для проведения - такой себе "экспонат".
	// Был списан практически втупую с конструктором запросов. В конце даже комментарии скопипащены!
	// Я подумываю, как бы лучше его отрефакторить по подобию общего модуля "ПроведениеРассчетов" - там ну мед же!
	// А пока пусть работает как есть.
	...
	// Люблю запрос1, запрос2, потом а, б, массив, данные, текст и прочие метаназвания.
	Запрос = Новый Запрос;
